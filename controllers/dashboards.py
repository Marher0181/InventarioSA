...
def dashboardOperador():
    ...
    if idRol == 2:
        ...
        sql = db.text("SELECT COUNT(*) FROM Categorias")
        categorias = db.session.execute(sql).fetchall()
        return render_template('DashboardAdmin.html', ventas=ventas, proveedores=proveedores, usuarios=usuarios, movimientos=movimientos, productos=productos, categorias=categorias)
